...
Для виводу графіки було вирішино писати напряму у відеопам'ять (0B800h), а не через BIOS-переривання на кожен символ. Завдяки цьому екран оновлюється миттєво - без мерехтіння при перемальовуванні поля чи русі курсора. Усі фігури намальовані кастомними 8-бітними шрифтами (sprites.inc), які завантажуються в пам'ять VGA - виглядає як справжня ретро-гра.

З фонами зробили так: написала Python-скрипт (make_bg.py), який конвертує картинки під 16-колірну DOS-палітру через зважену RGB-відстань і генерує .inc файли з байтовими масивами. Вони одразу вшиваються в бінарник при збірці в TASM. Ніяких затримок при старті, розмір менший.
...
Ми обрали модель пам'яті Small (.MODEL small), тому програма розбита на сегменти по 64 КБ максимум. Схема виглядає так:

![Схема організації пам'яті проєкту](memory_diagrama.png)

- CS (код): всі скомпільовані модулі в одному місці - main.obj, display.obj, game.obj, input.obj, ai.obj.
- DS (дані): глобальні змінні і стан гри. Основне:
    - board DB 64 DUP(?) - дошка 8x8 як одновимірний масив, клітинка - row * 8 + col
    ...
    - таблиці напрямків (rook_dirs, bishop_dirs, knight_offsets, king_dirs) - константи для генерації ходів
    - bg_data DB 4000 DUP(0) - буфер під фонове зображення (символи + кольорові атрибути)
- SS (стек): виділила 4096 байт - вистачає для адрес повернення і локальних параметрів. Координати фігур передаю через bp у модулі логіки.
- ES (відеопам'ять): жорстко прибита до 0B800h. Пишу напряму — без переривань, без гальм, екран оновлюється миттєво.
